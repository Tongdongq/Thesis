...
Figure \ref{fig:dna_structure} shows a schematic overview of the components of DNA.
...
\figC{scale=.9}{pyro_sequencing_1.jpg}{Workflow of pyrosequencing, source: \cite{pyro_sequencing_fig}}{fig:pyro_sequencing}
...
Ion Torrent produces reads of up to 400 basepairs, but the error rate is relatively high at 1.5\%.
...
Another advantage is that it produces some of the longest read lengths in the industry, with an average of 10000 basepairs, and longest reads of 60000 basepairs \cite{pacbio_readlength}, Figure \ref{fig:pacbio_readlength} shows a distribution of the readlengths.
...
A new application of SMRT sequencing includes adding single-stranded loops to a double-stranded template, creating a circular template called a SMRTbell \cite{SMRTbell}, shown in Figure \ref{SMRTbell}.
...
\todo{should dynamic programming be a separate subsection? Could be an appendix as well}
...
\todo{cite/prove that NW is proven optimal?}
...
\figC{width=.6\textwidth}{NW1.png}{The NW matrix, initialised, source: \cite{NW_online}}{fig:NW1}
...
% Ant colony optimzation, particle swarm optimization
% BWA
...
There are two types of assembly: denovo and reference based.
Denovo assembly works as described above, but reference based assembly first maps the reads to a reference.
If multiple reads map to the same place in the reference, they are likely to be overlapping.
Denovo assembly compares each read with every other read, resulting in a naive time complexity of $O(N^2)$, where N is the number of reads.
Denovo assembly is an NP-hard problem \cite{denovo_NP}.
...
During the layout phase, the graph is analysed to find paths, corresponding to segments of the original genome.
...
\todo{consider talking about Optical Mapping to aid scaffolding}
\todo{talk about error correction before/after assembly?}
...
An uncommon approach is to use sorting to find matching Kers, like in merge-sort \cite{mergesort}\cite{sorting}.
...
\todo{maybe divide into subsections}
\section{CUDA}
...
\todo{add picture of hierarchy, possible source in comments}
...
\todo{add section on OpenCL?}
%\section{OpenCL}




%\todo{include this section? this work mainly focuses on GPU computation, instead of heterogeneous computation}
...
\todo{mention other/more GPU algorithms?}
