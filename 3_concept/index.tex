...
Figures \ref{fig:edit_graph1} and \ref{fig:edit_graph2} show images from a tool, created by Nicholas Butler \cite{edit_graph}, that shows how the edit graph is traversed by the O(nd) algorithm.

\figC{width=.7\textwidth}{edit_graph.png}{An empty edit graph, source: \cite{edit_graph}}{fig:edit_graph}
\figC{width=.7\textwidth}{edit_graph2.png}{A filled edit graph, the dark blue lines represent mismatches and their subsequent snakes, the light blue lines are the d-waves, the brown lines are odd diagonals, yellow lines are even diagonals, the red path is the shortest path, source: \cite{edit_graph}}{fig:edit_graph2}
