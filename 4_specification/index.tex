...
This buffer could be coalesced, but the tracebackprocedure takes only a fraction of the GPU time.
...
The matrix with tracebackpointers is stored in an array, this allows them to be coalesced.
Each tracebackpointer occupies one byte, although it would be possible to store two pointers in one byte.
